...
โปรเจกต์นี้อยู่ **ใน Git repo** ไม่ได้อยู่ที่ `C:\DashboardWebApp` จนกว่าจะคัดลอกเอง

| ที่ต้องการเปิด | path จริง |
|----------------|-----------|
| โฟลเดอร์โปรเจกต์ใน repo | `DashboardWebApp\` (รากของ repo ไม่ใช่ใน `test\`) |
| ไฟล์ Visual Studio | `DashboardWebApp.sln` (รากของ repo) |
| หน้าเริ่มต้น | `DashboardWebApp\index.aspx` |
| ไฟล์ Excel ตัวอย่าง | `DashboardWebApp\App_Data\test4.xlsx` |
| วางที่ไดรฟ์ C: | ดับเบิลคลิก `คัดลอกไป-C-DashboardWebApp.bat` แล้วเปิด `C:\DashboardWebApp\DashboardWebApp.sln` |

หลัง clone บน Windows ตัวอย่าง path:

```
C:\Users\<ชื่อคุณ>\source\repos\camera\DashboardWebApp.sln
C:\Users\<ชื่อคุณ>\Downloads\camera-main\DashboardWebApp.sln
```

อย่าหาที่ `C:\test` หรือ `C:\DashboardWebApp` ถ้ายังไม่ได้รันไฟล์ `.bat`
...
ใน Git repo โปรเจกต์อยู่ที่ราก:

```
camera\                         ← โฟลเดอร์หลัง clone
├── หาแดชบอร์ดที่นี่.txt
├── คัดลอกไป-C-DashboardWebApp.bat
├── DashboardWebApp.sln         ← เปิดไฟล์นี้ใน Visual Studio 2026
└── DashboardWebApp\
    ├── index.aspx
    └── App_Data\test4.xlsx
```

คัดลอกไปไดรฟ์ C: ด้วยการดับเบิลคลิก `คัดลอกไป-C-DashboardWebApp.bat`
...
```bash
python3 DashboardWebApp/scripts/generate_excel.py
```
...
**วิธีเร็ว:** ดับเบิลคลิก `คัดลอกไป-C-DashboardWebApp.bat` ที่รากของ repo

**วิธีมือ:**

1. สร้างโฟลเดอร์ `C:\DashboardWebApp`
2. คัดลอก `DashboardWebApp.sln` และโฟลเดอร์ `DashboardWebApp\` ไปไว้ใน `C:\DashboardWebApp`
3. ตรวจว่ามีไฟล์เหล่านี้:
   - `C:\DashboardWebApp\DashboardWebApp.sln`
   - `C:\DashboardWebApp\DashboardWebApp\index.aspx`
   - `C:\DashboardWebApp\DashboardWebApp\App_Data\test4.xlsx`
